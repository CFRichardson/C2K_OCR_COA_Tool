...
should be broken down in full script to increase adjustability & repairability
"""
...
def img_scope(whole_img, x,y,w,h, x_adjust=0, y_adjust=0, bs=(350,120), be=(480,190)):
    '''
    All 3 strips (left, middle, and right) have different x_s, x_e, etc!
    Parameters
        ----------
        ...
        Returns
        -------
        scope: which is the specific area of wanted label strip containing KEYS
    ...
    scope = whole_img[y + y_s : y + h + y_e, x + x_s :x + w + x_e].copy()
...
def return_mask(img_scope):
    '''
    Parameters
        ----------
        img_scope : specific area of label containing KEYS
    ...
    imgHSV = cv.cvtColor(img_scope, cv.COLOR_BGR2HSV)
    # create the Mask
...
# def return_chars(whole_img, x,y,w,h, x_s=410, x_e=730, y_s=90, y_e=110):
#     '''
#     Parameters
#         ----------
#         whole_img : whole_scan
...
#     # all cropped keys have a trailing section with useless info
#     rectangle = np.zeros((100,50), dtype='uint8')
#     scope = cv.rectangle(scope, (350,150), (460,190), (60,60,60), -1)
#
#     # Define lower and uppper limits for the color spectrum of white
#     lower = np.array([0, 0, 155])
#     upper = np.array([180, 40, 255])
